...
def test_chat_stream_emits_normalized_events(monkeypatch):
    ...
    class Snapshot:
        config = {"configurable": {"thread_id": "test_123", "checkpoint_id": "cp-1", "checkpoint_ns": ""}}
        values = {"messages": ["a", "b"], "route_history": [1, 2], "streaming_status": "completed"}
        next = ()
...
def test_chat_stream_direct_supervisor_response_uses_same_text_contract(monkeypatch):
    ...
    class Snapshot:
        config = {"configurable": {"thread_id": "direct_1", "checkpoint_id": "cp-direct", "checkpoint_ns": ""}}
        values = {"messages": ["user", "assistant"], "route_history": [], "streaming_status": "completed"}
        next = ()
        created_at = "2026-03-11T00:00:00+00:00"
    ...
    with client.stream("POST", "/api/chat", json={"message": "hello", "thread_id": "direct_1"}) as response:
        payloads = _sse_payloads(response)

    assert any(payload["event_type"] == "text" for payload in payloads)
    assert any(
        payload["event_type"] == "text" and "hello from supervisor" in payload["content"]
...
def test_chat_stream_resume_same_thread_id_restores_checkpoint_state(monkeypatch):
    ...
    with client.stream("POST", "/api/chat", json={"message": "first", "thread_id": "resume_1"}) as response:
        first_payloads = _sse_payloads(response)

    with client.stream("POST", "/api/chat", json={"message": "second", "thread_id": "resume_1"}) as response:
        second_payloads = _sse_payloads(response)

    first_text = "".join(
        payload["content"] for payload in first_payloads if payload["event_type"] == "text"
    )
    second_text = "".join(
        payload["content"] for payload in second_payloads if payload["event_type"] == "text"
    )
