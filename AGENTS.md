...
- 새 `plans/*.md` 파일을 만들거나 기존 계획서를 크게 갱신할 때는 문서 최상단에 사람이 빠르게 내용을 파악할 수 있는 간단한 요약 표 형태의 키-값 블록을 둔다.
- 이 상단 요약 블록에는 최소한 `작업명`, `간단요약`, `작성일시`, `최종 수정일시`를 포함한다.
- 권장 예시:

```md
---
작업명: Final Response Stream Duplication Refactor Plan
간단요약: 최종 응답 스트림 중복 전송 원인을 제거하고 관련 회귀 테스트를 보강한다.
작성일시: 2026-03-24 09:49 KST
최종 수정일시: 2026-03-24 09:49 KST
---
```

- 기존 계획서에 타임스탬프가 없다면, 해당 파일을 수정하는 시점에 함께 보강한다.
- 기존 계획서에 상단 요약 블록이나 타임스탬프가 없다면, 해당 파일을 수정하는 시점에 함께 보강한다.
...
- 어떤 변경이든 커밋 전에 반드시 검증 프로세스를 거쳐야 한다.
- 최소 기준:
  - 백엔드 변경은 관련 `pytest` 테스트 실행
  - 프런트엔드 변경은 관련 테스트 실행
  - UI/빌드 영향이 있는 프런트 변경은 `npm run build`까지 확인
- 검증이 실패한 상태에서는 커밋하지 않는다.
- 커밋은 반드시 “검증 통과 후” 진행한다.
