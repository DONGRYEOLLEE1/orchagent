...
- [x] 1.6 `services/event_recording_service.py` 신설. `EventRecordingService.record_session_event()` + `record_usage()` 두 façade가 JsonLogger 위에 통합 인터페이스 제공. chat.py가 두 메서드로 통일된 trace/analytics/json_log 단일 호출 보유. 회귀 0.
...
- [x] 2.2 **휴리스틱 제거 Phase A — 정규식·`_should_force_*` 함수 삭제**. supervisor.py에서 모든 정규식 + 모든 `_should_force_*` 함수를 제거하고, 호출자(supervisor 본체)에서도 사용을 정리. text_keywords.py 신설은 불필요(라우팅 외 trace 용도 사용처 0건 확인). **3 라운드로 진행 완료**.
  - [x] 2.2 라운드 1 — vision/coding/data force-route 블록 + 직접 의존 휴리스틱 호출자만 제거. supervisor.py: L489 `_should_force_coding_team` requires_approval=False 보조 분기, L771-842 force-route 4종(coding_team / data_science_team / attachment→FINISH / vision_team), L935-952 shared_context 자동 기록 3종(vision_routed/data_science_routed/coding_routed). 989 → 896 LOC(-93). 함수 정의(`_should_force_*`, `_latest_user_request_has_image`, patterns, `_dispatched_team_workers`)와 팀별 강제 순서 머신 / `_should_force_approval` / safeguard `allowed_next_nodes` coercion은 보존(후속 라운드). 테스트 마이그레이션: `test_supervisor.py`에서 force-route 의존 테스트 6개 삭제(`test_head_supervisor_forces_vision_team_before_direct_finish_for_image_turn`, `test_head_supervisor_does_not_loop_back_to_vision_after_vision_turn`, `test_head_supervisor_forces_data_science_team_for_file_analysis_turn`, `test_head_supervisor_does_not_loop_back_to_data_science_team`, `test_head_supervisor_does_not_route_attachment_turn_to_research_after_data_science`, `test_head_supervisor_does_not_route_attachment_turn_to_writing_after_data_science`) + `test_coding_supervisor.py::test_head_supervisor_routes_repo_bound_coding_request_to_coding_team` 1개 삭제. **삭제된 7개는 LLMRouter(Phase 2.4) 적용 후 LLM 프롬프트 회귀 테스트로 별도 신설 예정**. pytest 331 → **324 PASS**(-7, 회귀 0).
  - [x] 2.2 라운드 3 — supervisor.py에서 휴리스틱 함수 + patterns 완전 제거. 제거: 5개 patterns(`_APPROVAL_PATTERNS`/`_DATA_ANALYSIS_PATTERNS`/`_CODING_PATTERNS`/`_RUNTIME_VERIFY_PATTERNS`/`_CODING_EDIT_INTENT_PATTERNS`), 8개 함수(`requires_human_approval_for_text`/`requires_coding_team_for_text`/`requires_coding_edit_for_text`/`_should_force_approval`/`_shared_context_has_data_attachments`/`_should_force_data_science_team`/`_shared_context_has_repo_binding`/`_should_force_coding_team`/`_content_contains_image`/`_latest_user_request_has_image`), `import re`, supervisor_node 내부 `heuristic_requires_approval` 변수 + 가드 출력문. `force_requires_approval` shared_context 분기는 HITL 안전망으로 보존. supervisor.py 588 → **431 LOC**(-157). planner.py: `requires_coding_team_for_text` import + `_build_simple_coding_plan` helper 제거(planner LLM이 PLANNER_PROMPT L162 가이드에 따라 자체 판단). orchestration_service.py: 키워드 패턴을 inline화(`_CODING_PREFLAG_PATTERNS`/`_APPROVAL_PREFLAG_PATTERNS`) — chat.py L548/L635에서 워크스페이스 materialization gating + `force_requires_approval` seed 용도로 정당 사용 중이므로 메서드 자체는 보존(supervisor 라우팅과 무관한 API-layer pre-flag). 테스트 마이그레이션: `test_supervisor.py`에서 휴리스틱 의존 4개 삭제(`test_head_supervisor_forces_approval_for_filesystem_write_requests`, `test_head_supervisor_forces_approval_for_code_execution_requests`, `test_head_supervisor_forces_approval_for_tuple_user_messages`, `test_requires_human_approval_for_text_detects_risky_requests`). `test_head_supervisor_forces_approval_from_shared_context_flag`는 state-flag 분기 검증으로 보존. pytest 310 → **306 PASS**(-4, 회귀 0). `grep -rn "_should_force_\|_APPROVAL_PATTERNS\|_CODING_PATTERNS\|_DATA_ANALYSIS_PATTERNS\|_RUNTIME_VERIFY_PATTERNS\|_CODING_EDIT_INTENT_PATTERNS" packages/agent-core --include="*.py"` 결과 docstring 2건만 잔존(router_schema.py L17/L49 — 휴리스틱 대체 설명 문장).
- [x] 2.3 **휴리스틱 제거 Phase B — 팀별 강제 순서 머신 삭제 완료**(commit c56c42a / PR #7). supervisor.py에서 coding/data_science/research 강제 순서 머신(121 LOC) + plan-driven override(48 LOC) 제거. team supervisor가 LLM 결정만으로 dispatched_workers 인식 + worker 선택. 14개 의존 테스트 마이그레이션.
  - [x] 2.3 라운드 2 — 팀별 강제 순서 머신 3종(data_science / research / coding) + head plan-driven override + plan-driven 의존 휴리스틱 함수들을 일괄 제거. supervisor.py: data_science 머신(~25 LOC), research 머신(~29 LOC), coding 머신(~67 LOC), head plan-driven override 블록(~48 LOC), inner `_head_redirects_for` + 별도 final guard를 `SAFEGUARDS.head_team_redirect_limit` 사용 단일 streak 계산 safeguard로 정리. 모듈 레벨 함수 제거: `_extract_team_stage_sequence`, `_extract_completed_team_sequence`, `_next_pending_team_stage`, `_messages_contain_chart_artifact_evidence`, `_latest_message_signals_review_passed/failed`, `_latest_user_requested_visualization`, `_latest_user_requested_runtime_verification`, `_latest_user_requests_coding_edit`, `_dispatched_team_workers` (총 10개). **보존**: `_should_force_approval`(L184) / L348·L358 호출 + coding_team-without-repo final guard + `allowed_next_nodes` coercion + `max_team_dispatches` dispatch guard + `_should_force_data_science_team`, `_should_force_coding_team`, `_shared_context_has_*`(라운드 3 대상), patterns + `requires_*_for_text` 4종(planner.py L93 등에서 사용). supervisor.py 896 → **588 LOC**(-308). 테스트 마이그레이션: `test_supervisor.py`에서 plan-driven override 의존 4개 + data_science 강제순서 4개 + research 강제순서 3개 = 11개 삭제. `test_coding_supervisor.py` 강제순서 2개 삭제 → 파일을 placeholder docstring으로 대체. `test_supervisor_progression.py`의 단일 plan-completion 테스트(`test_head_supervisor_treats_duplicate_completed_team_entries_as_complete`)도 plan-driven override 의존이라 삭제(파일 placeholder docstring 대체). pytest 324 → **310 PASS**(-14, 회귀 0). 삭제 14개는 Phase 2.8 routing evaluation harness + LLM 프롬프트 회귀 테스트(2.7)로 대체될 예정.
- [x] 2.4 `supervisor_node()` 단순화 — `agent_core/supervisors/` 디렉토리 신설(`__init__.py` 22 LOC, `llm_router.py` 157 LOC, `head_supervisor.py` 344 LOC, `team_supervisor.py` 175 LOC). `llm_router.decide_route()`가 `RouterDecision` structured-output + safeguard chain(reject_invalid_goto → enforce_team_redirect_limit → enforce_dispatch_limit) + parse-failure fallback을 한 함수로 묶음. head/team factory는 프롬프트 + 레이어-specific 책임(HITL interrupt, identity override, finalizer 선택 / dispatch counter)만 가짐. supervisor.py는 75 LOC 어댑터로 단순화(legacy `make_supervisor_node` 시그니처 + helper 재export 보존 → main_graph.py / builder.py / 12 test 무변경). pytest 306 → **313 PASS**(+7 test_llm_router.py, 회귀 0).
  - [x] 2.4 회귀 fix — `RouterDecision.content` 필드 누락으로 head supervisor가 FINISH로 직접 답변하는 simple turn(예: "한 문장으로 자기소개 해주세요.")에서 빈 AIMessage 발생. `router_schema.RouterDecision`에 `content: str = ""` 추가(+9 LOC), `head_supervisor` direct-FINISH 경로에서 identity override 미적용 시 `decision.content`를 promote(+5 LOC), `fragments.ROUTER_DECISION_GUIDANCE` JSON 예시·가이드에 `content` 명시(+1 LOC delta), `prompts.SYSTEM_SUPERVISOR_PROMPT` 가이드 #3·#4를 RouterDecision 스키마에 정합(message channel 가정 제거 + simple turn JSON 예시 추가, f-string × format 호환 위해 4중 brace escape `{{{{}}}}`). safeguard force-FINISH는 content 기본값 `""` 그대로 유지(작성 권한 없음). 테스트 추가 — `test_llm_router.py`: content round-trip / safeguard-strip 2 케이스 + 기존 valid/finish 케이스에 content="" assertion. `test_supervisor.py`: identity 패턴 미매칭 simple turn에서 LLM content가 AIMessage로 emit되는지 1 케이스. pytest 313 → **316 PASS**(+3, 회귀 0).
...
- [x] 2.7 supervisor / team-supervisor 프롬프트 강화 — `SYSTEM_SUPERVISOR_PROMPT`(v2.5 → v2.6)에 `ROUTER_DECISION_GUIDANCE` 통합 + `TEAM SELECTION HINTS` 블록(이미지→vision_team / 데이터 attachment→data_science_team / repo binding+코드 의도→coding_team) + 가이드 필드명을 RouterDecision 스키마(`reason`/`request_review`)에 맞춤. `TEAM_SUPERVISOR_PROMPT`(v1.2 → v1.3)에 `ROUTER_DECISION_GUIDANCE` + `WORKER REUSE POLICY` 블록(route_history 검토 + 중복 dispatch 금지 + `team_finished` 시그널 사용) 추가. fragments는 f-string으로 임베드, JSON 예시의 `{}`는 `{{}}`로 escape하여 `str.format(members=…)` 호환.
...
- [x] 2.13 **Phase 2 통합 회귀 완료**(PR #7 머지, 태그 `refactor-phase-2-complete`). pytest 316 PASS(275 baseline → +41, 회귀 0). LLMRouter 단위 검증 7케이스 + Direct-Answer 회귀 fix 검증 3케이스. E2E S1(simple chat direct FINISH)·S2(research_team via LLM-driven routing → Tavily search → "Welcome to Python.org") 모두 PASS — playwright MCP 사용자 관점 검증.
...
- [x] 3.2 커스텀 훅 4개 추출 완료(commit 8bfd3e3 / PR #8). useThreadCollection(294 LOC) / useActiveThread(155) / useStreamSession(138) / useActionSpace(156). 각 hook당 vitest 2 케이스(총 8). page.test.tsx 모든 기존 케이스 호환.
...
- [x] 4.5 프롬프트 공통 fragment 모듈 신설 — `packages/prompt-kit/src/prompt_kit/fragments.py`에 `CRITICAL_GUIDELINES`/`WORKER_CONSTRAINTS`/`ROUTER_DECISION_GUIDANCE`(Phase 2.7 routing 지침)을 정의. Phase 2.4/2.7와 함께 `prompts.py`가 fragments를 import → `SYSTEM_SUPERVISOR_PROMPT` / `TEAM_SUPERVISOR_PROMPT`에 `ROUTER_DECISION_GUIDANCE`를 임베드 + 재-export(`__all__`)로 단일 출처화. `CRITICAL_GUIDELINES`/`WORKER_CONSTRAINTS`는 도메인-specific 워커 가이드와 충돌하지 않도록 재-export만 우선 적용(향후 일반 워커 추가 시 직접 활용).
...
- [x] 5.0 모든 phase 브랜치 main 머지 완료. `git log --oneline --merges main`: Phase 0(615db52) + Phase 1(9ef19b7) + Phase 2 partial(d7f9b5f) + Phase 2 LLM-Driven Routing(87d143b) + Phase 3(f78fa80). 태그 5개(`refactor-phase-{0,1,2-partial,2,3}-complete`) 모두 origin push.
...
- [x] 5.7 본 plan 모든 phase 체크박스 `- [x]` 확인 완료. 상단 "최종 수정일시: 2026-05-21 13:05 KST" 갱신. 누적 진척: Phase 0~3 + Phase 4 + Phase 5의 모든 sub-task 완료. supervisor.py 989→75 LOC(-92%), WorkspaceRouteRoot.tsx 2,626→1,490 LOC(-43%), pytest 275→316(+41), vitest 53→88(+35), 회귀 0.
