...
- [ ] 새 thread에서 긴 질문을 보내고 목록 title이 짧은 AI 제목으로 바뀌는지 확인한다.
- [ ] chat 응답이 길게 걸려도 title이 먼저 반영될 수 있는지 확인한다.
- [ ] manual rename 후 다시 첫 질문 title이 덮어써지지 않는지 확인한다.
- [ ] 브라우저 새로고침 후에도 AI 제목이 유지되는지 확인한다.

### Phase 7. Playwright MCP 최종 검증

- [ ] 실제 브라우저에서 새 thread를 시작한다.
- [ ] `웹검색을 통해 RoPE 논문을 탐색하고 메인 연구자가 원하는 바는 무엇인지 설명해주세요.` 를 입력한다.
- [ ] 채팅 응답 완료를 기다리지 않고도 thread title이 짧은 AI 제목으로 바뀌는지 관찰한다.
- [ ] title이 질문 원문 truncate가 아니라 목적 중심 요약인지 확인한다.
- [ ] 유사 질의 1~2개를 더 실행해 한국어 title 품질과 길이 제한을 확인한다.
- [ ] DB에서 해당 thread의 `thread_profiles.title_override`가 기대값으로 저장됐는지 확인한다.
...
- [ ] 새 thread 첫 질문에서만 AI 제목이 생성된다.
- [ ] 제목 생성이 chat stream을 블로킹하지 않는다.
- [ ] 제목이 한국어 중심의 짧은 한 줄 요약으로 들어간다.
- [ ] manual rename 이후에는 AI 제목이 덮어쓰지 않는다.
